...
import { signageStore } from "../../contexts/store";
...
export default function Dashboard() {
  ...
  const P = (w:"400"|"500"|"600"|"700") => `Poppins_${w === "400"?"400Regular":w==="500"?"500Medium":w==="600"?"600SemiBold":"700Bold"}`;
  ...
  const toggle = (id: string) => {
    setSel(prev => prev.includes(id)
      ? prev.filter(x => x !== id)
      : prev.length < layout.slots ? [...prev, id] : [...prev.slice(1), id]);
  };

  const pickImg = async () => {
    const r = await ImagePicker.launchImageLibraryAsync({ allowsMultipleSelection: true });
    if (!r.canceled) setImages(p => [...r.assets.map((a,i)=>({ id:`c${Date.now()}${i}`, uri:a.uri, label:`Custom ${p.length+i+1}`, tag:"custom" })), ...p]);
  };
...
const doSend = async () => {
  ...
  console.log("Starting send process...", selImgs);

  const BIN_ID = "69cf9d46856a682189f6fd6a";
  const API_KEY = "$2a$10$Efm/UAU.d8QJEPXSqWD.weXXqJoyK9vlSUK.TsKSF5PuOmWWJqmm2";

  try {
    // ✅ NO base64 (very important)
    const imageUris = selImgs.map(i => i.uri);

    console.log("Sending URIs:", imageUris);
    ...
    console.error("❌ Error sending to JSONBin:", error);
  }
};
  ...
  const Preview = () => (
    <View style={[pv.wrap, { aspectRatio: ratio.v }]}>
      <View style={pv.bar}>
        {[B,"#3B82F6","#22C55E"].map((c,i)=><View key={i} style={[pv.dot,{backgroundColor:c}]}/>)}
        <Text style={[pv.barT,{fontFamily:P("400")}]}>Live Preview · {ratio.l} · {layout.label}</Text>
        <View style={pv.liveBadge}><View style={pv.liveDot}/><Text style={[pv.liveT,{fontFamily:P("600")}]}>LIVE</Text></View>
      </View>
      <View style={{ flex:1, flexDirection:"row", flexWrap:"wrap" }}>
        {Array.from({ length: layout.cols * layout.rows }).map((_,i) => {
          const img = selImgs[i];
          return (
            <View key={i} style={{ width:`${100/layout.cols}%`, height:`${100/layout.rows}%` }}>
              {img
                ? <Image source={{ uri:img.uri }} style={{ width:"100%", height:"100%" }} resizeMode="cover"/>
                : <View style={pv.empty}><Text style={[pv.emptyT,{fontFamily:P("400")}]}>Slot {i+1}</Text></View>
              }
            </View>
          );
        })}
      </View>
      {/* Overlay title on preview */}
      {title ? (
        <View style={pv.titleOverlay}>
          <Text style={[pv.titleOverlayT,{fontFamily:P("700")}]} numberOfLines={1}>{title}</Text>
          {msg ? <Text style={[pv.msgOverlayT,{fontFamily:P("400")}]} numberOfLines={1}>{msg}</Text> : null}
        </View>
      ) : null}
    </View>
  );
  ...
  return (
    <ScrollView style={s.root} showsVerticalScrollIndicator={false}>
      <View style={[s.wrap, mob && s.wrapM]}>
        ...
        <View style={s.nav}>
          <View style={s.navL}>
            <View style={s.navLogo}><Image source={require("../../assets/images/logo.png")} style={s.navLogoImg} resizeMode="contain"/></View>
            <View>
              <Text style={[s.navBrand,{fontFamily:P("700")}]}>Screenova</Text>
              <Text style={[s.navTag,{fontFamily:P("600")}]}>ADMIN PORTAL</Text>
            </View>
          </View>
          <View style={s.navR}>
            <View style={s.adminPill}>
              <View style={s.adminDot}/>
              <Text style={[s.adminT,{fontFamily:P("600")}]}>admin</Text>
            </View>
            <TouchableOpacity style={s.logoutBtn} onPress={()=>router.replace("/login")}>
              <Text style={[s.logoutT,{fontFamily:P("600")}]}>Logout</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={s.pageTitleRow}>
          <View>
            <Text style={[s.pageTitle,{fontFamily:P("700")}]}>Dashboard</Text>
            <Text style={[s.pageSub,{fontFamily:P("400")}]}>Build your signage · Preview · Send to TV</Text>
          </View>
          <View style={s.statRow}>
            {[{n:"12",l:"Assets"},{n:sel.length.toString(),l:"Selected"},{n:"4K",l:"Display"}].map(({n,l})=>(
              <View key={l} style={s.stat}>
                <Text style={[s.statN,{fontFamily:P("700")}]}>{n}</Text>
                <Text style={[s.statL,{fontFamily:P("400")}]}>{l}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[s.main, mob && s.mainCol]}>
          ...
          <View style={[s.card, { flex: mob ? undefined : 1.35 }]}>
            <View style={s.cardHead}>
              <Text style={[s.cardTitle,{fontFamily:P("700")}]}>Media Library</Text>
              <TouchableOpacity style={s.uploadBtn} onPress={pickImg}>
                <Text style={[s.uploadT,{fontFamily:P("600")}]}>+ Upload</Text>
              </TouchableOpacity>
            </View>
              ...
              <View style={s.tagsRow}>
                {TAGS.map(t=>(
                  <TouchableOpacity key={t} style={[s.tagBtn, tag===t && s.tagA]} onPress={()=>setTag(t)}>
                    <Text style={[s.tagT,{fontFamily:P("600")}, tag===t && s.tagTA]}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>

            <Text style={[s.selInfo,{fontFamily:P("400")}]}>
              Select up to <Text style={{fontFamily:P("600"),color:B}}>{layout.slots}</Text> image{layout.slots>1?"s":""} · {sel.length} chosen
            </Text>
            ...
            <View style={s.imgGrid}>
              {filtered.map(img => {
                ...
                return (
                  <TouchableOpacity key={img.id} style={[s.imgCell, active && s.imgCellA]} onPress={()=>toggle(img.id)}>
                    <Image source={{ uri:img.uri }} style={s.imgThumb} resizeMode="cover"/>
                    {active && <View style={s.imgBadge}><Text style={s.imgBadgeT}>{idx+1}</Text></View>}
                    {active && <View style={s.imgCheckOverlay}/>}
                    <View style={s.imgFooter}>
                      <Text style={[s.imgFooterT,{fontFamily:P("500")}]} numberOfLines={1}>{img.label}</Text>
                      <View style={[s.imgTagPill,{backgroundColor: active?B+"22":"#F1F5F9"}]}>
                        <Text style={[s.imgTagT,{fontFamily:P("600"),color:active?B:G}]}>{img.tag}</Text>
                      </View>
                    </View>
                  </TouchableOpacity>
                );
          ...
          {/* ── RIGHT: Controls + Preview + Send ── */}
          <View style={{ flex:1, gap:16 }}>

            {/* Content Info */}
            <View style={s.card}>
              <Text style={[s.cardTitle,{fontFamily:P("700")}]}>Content Info</Text>
              <Text style={[s.fieldLbl,{fontFamily:P("600")}]}>Display Title</Text>
              <TextInput
                style={[s.input, foc==="t" && s.inputF, {fontFamily:P("400")}]}
                value={title} onChangeText={setTitle}
                placeholder="e.g. Morning Motivation Ritual"
                placeholderTextColor={G}
                onFocus={()=>setFoc("t")} onBlur={()=>setFoc(null)}
              />
              <Text style={[s.fieldLbl,{fontFamily:P("600")},{marginTop:12}]}>Message / Location</Text>
              <TextInput
                style={[s.input, foc==="m" && s.inputF, {fontFamily:P("400")}]}
            ...
            {/* Aspect Ratio */}
            <View style={s.card}>
              <Text style={[s.cardTitle,{fontFamily:P("700")}]}>Aspect Ratio</Text>
              <View style={s.pillRow}>
                {RATIOS.map(r=>(
                  <TouchableOpacity key={r.l} style={[s.pill, ratio.l===r.l && s.pillA]} onPress={()=>setRatio(r)}>
                    <Text style={[s.pillT,{fontFamily:P("600")}, ratio.l===r.l && s.pillTA]}>{r.l}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            {/* Layout */}
            <View style={s.card}>
              <Text style={[s.cardTitle,{fontFamily:P("700")}]}>Screen Layout</Text>
              <View style={s.layoutGrid}>
                {LAYOUTS.map(l => {
                  const active = layout.label===l.label;
                  return (
                    <TouchableOpacity key={l.label} style={[s.layoutCell, active && s.layoutCellA]}
                      onPress={()=>{ setLayout(l); setSel([]); }}>
                      <LayoutIcon cols={l.cols} rows={l.rows} active={active}/>
                      <Text style={[s.layoutT,{fontFamily:P("600")}, active && s.layoutTA]}>{l.label}</Text>
                      <Text style={[s.layoutSub,{fontFamily:P("400")}, active && {color:"#ffffff99"}]}>{l.slots} slot{l.slots>1?"s":""}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
            ...
            <View style={s.card}>
              <View style={s.cardHead}>
                <Text style={[s.cardTitle,{fontFamily:P("700")}]}>Live Preview</Text>
                <View style={s.resBadge}>
                  <Text style={[s.resBadgeT,{fontFamily:P("600")}]}>4K Ready</Text>
                </View>
              </View>
              <Preview/>
            </View>
            ...
            <Animated.View style={{ transform:[{scale:pulse}] }}>
              <TouchableOpacity
                style={[s.sendBtn, !sel.length && s.sendOff]}
                onPress={doSend} activeOpacity={0.86}
              >
                {sent
                  ? <><Text style={s.sendIcon}>✓</Text><Text style={[s.sendT,{fontFamily:P("700")}]}>Sent to TV!</Text></>
                  : <><Text style={s.sendIcon}>📺</Text><Text style={[s.sendT,{fontFamily:P("700")}]}>Send to TV  ({sel.length}/{layout.slots})</Text></>
                }
              </TouchableOpacity>
            </Animated.View>
            ...
            {sent && (
              <View style={s.sentRow}>
                ...
                <Text style={[s.sentT,{fontFamily:P("500")}]}>TV display updated · {new Date().toLocaleTimeString()}</Text>
              </View>
            )}
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  pageTitleRow: { flexDirection:"row", justifyContent:"space-between", alignItems:"center", marginBottom:24 },
